...
export class AgeOfSigmarActorSheet extends ActorSheet {
  ...
  async _handleEnrichment() {
    let enrichment = {};
    enrichment["system.bio.background"] = await TextEditor.enrichHTML(
      this.actor.system.bio.background,
      { async: true, secrets: this.actor.isOwner, relativeTo: this.actor }
    );
    enrichment["system.bio.connections"] = await TextEditor.enrichHTML(
      this.actor.system.bio.connections,
      { async: true, secrets: this.actor.isOwner, relativeTo: this.actor }
    );
    enrichment["system.notes"] = await TextEditor.enrichHTML(
  ...
  _addWoundImages(sheetData) {
    sheetData.system.combat.wounds.map((i) => {
      switch (i.type) {
        case "minor":
          i.img = "icons/skills/wounds/blood-spurt-spray-red.webp";
          break;
        case "serious":
          i.img = "icons/skills/wounds/injury-triple-slash-bleed.webp";
          break;
        case "deadly":
          i.img = "icons/skills/wounds/injury-pain-body-orange.webp";
          break;
        default:
          i.img = "icons/skills/wounds/blood-spurt-spray-red.webp";
          break;
  ...
  _addPowerBar(sheetData) {
    sheetData.system.power.pct =
      (sheetData.system.power.consumed / sheetData.system.power.capacity) * 100;
    if (sheetData.system.power.pct <= 30) sheetData.system.power.state = "low";
  ...
  constructItemLists(sheetData) {
    ...
    items.goals = this.actor
      .getItemTypes("partyItem")
      .filter((i) => i.category == "shortGoal" || i.category == "longGoal");
    ...
    items.equipped.weapons = this.actor
      .getItemTypes("weapon")
      .filter((i) => i.equipped);
    items.equipped.armour = this.actor
      .getItemTypes("armour")
      .filter((i) => i.equipped);

    items.attacks = items.equipped.weapons.concat(
      items.aethericDevices.filter((i) => i.damage && i.equipped)
    );
  ...
  constructEffectLists(sheetData) {
    ...
    effects.temporary = sheetData.actor.effects.filter(
      (i) => i.isTemporary && !i.disabled && !i.isCondition
    );
    effects.disabled = sheetData.actor.effects.filter((i) => i.disabled);
    effects.passive = sheetData.actor.effects.filter(
      (i) => !i.isTemporary && !i.disabled && !i.isCondition
    );
  ...
  _sortItemLists(items) {
    for (let list in items) {
      ...
      else if (typeof items[list] == "object") this._sortItemLists(items[list]);
    }
  }
        ...
        return Dialog.confirm({
          title: game.i18n.localize(
            this.actor.type == "player"
              ? "HEADER.CHARGEN"
              : "HEADER.ARCHETYPE_APPLY"
          ),
          content: `<p>${game.i18n.localize(
            this.actor.type == "player" ? "CHARGEN.PROMPT" : "ARCHETYPE.PROMPT"
          )}</p>`,
  ...
  activateListeners(html) {
    ...
    html
      .find(".item-dropdown-right")
      .contextmenu(this._onDropdownClick.bind(this));
  ...
  _onItemCreate(event) {
    ...
    let data = {
      name: `${game.i18n.localize("ITEM.NEW")} ${game.i18n.localize(
        CONFIG.Item.typeLabels[header.type]
      )}`,
      type: header.type,
    };

    if (data.type == "goal") data.type = "partyItem";

    if (header.category) data["system.category"] = header.category;
  ...
  async _onEffectCreate(ev) {
    ...
    let effectData = {
      label: game.i18n.localize("QUICKEFFECT.NEW"),
    ...
    let html = await renderTemplate(
      "systems/polaris-soulbound/template/dialog/quick-effect.hbs"
    );
  ...
  async _onSpellMiracleClick(event) {
    ...
    let test;
    if (item.type == "spell") test = await this.actor.setupSpellTest(powerId);
  ...
  _onTraitClick(ev) {
    ...
    this._dropdown(ev, {
      text: trait.description || game.aos.config.traitDescriptions[trait.name],
    });
  }
  ...
  async _dropdown(event, dropdownData) {
    ...
    if (li.hasClass("expanded")) {
      // If expansion already shown - switch to description
      ...
      if (!dropdownData) {
        ...
        dropdownHTML = `<div class="item-summary">${await TextEditor.enrichHTML(
          dropdownData.text
        )}`;
      }
