...
export class PartySheet extends AgeOfSigmarActorSheet {
  ...
  costructPartyItemLists(sheetData) {
    sheetData.party = {
      longGoals: sheetData.items.partyItems.filter(
        (i) => i.category == "longGoal"
      ),
      shortGoals: sheetData.items.partyItems.filter(
        (i) => i.category == "shortGoal"
      ),
      allies: sheetData.items.partyItems.filter((i) => i.category == "ally"),
      enemies: sheetData.items.partyItems.filter((i) => i.category == "enemy"),
      resources: sheetData.items.partyItems.filter(
        (i) => i.category == "resource" || !i.category
      ),
  ...
  _onItemCreate(event) {
    ...
    data.name = `${game.i18n.localize("ITEM.NEW")} ${
      game.aos.config.partyItemCategories[header.category]
    }`;
  ...
  async _onDrop(event) {
    ...
    if (document.documentName == "Item") return super._onDrop(event);

    if (document.type == "player" && document.hasPlayerOwner)
      this._addMembers(document);
    else if (document.type != "party") this._addAllyOrEnemy(document);
  }
  ...
  async _addAllyOrEnemy(actor) {
    let partyItem = {
      name: actor.name,
      type: "partyItem",
      "flags.polaris-soulbound.actorId": actor.id,
    };
    new Dialog({
  ...
  _onAllyEnemyClick(event) {
    ...
    if (actor && event.button == 0) actor.sheet.render(true);
