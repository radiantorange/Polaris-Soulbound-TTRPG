...
export class AgeOfSigmarActor extends Actor {
  async _preCreate(data, options, user) {
    if (data._id) options.keepId = SoulboundUtility._keepID(data._id, this);
    ...
    if (data.type === "player") {
      ...
      initData["flags.polaris-soulbound.autoCalcTokenSize"] = true;
    }
  ...
  async _preUpdate(updateData, options, user) {
    ...
    if (
      this.prototypeToken.texture.src.includes(
        "modules/soulbound-core/assets/tokens/unknown"
      ) &&
      updateData.img &&
      !updateData.token?.img
    ) {
    ...
    try {
      if (hasProperty(data, "system.combat.health.toughness.value"))
        this._displayScrollingChange(
          getProperty(data, "system.combat.health.toughness.value") -
            this.combat.health.toughness.value
        );
      if (hasProperty(data, "system.combat.mettle.value"))
        this._displayScrollingChange(
          getProperty(data, "system.combat.mettle.value") -
            this.combat.mettle.value,
          { mettle: true }
        );
    } catch (e) {
  ...
  prepareData() {
    ...
    if (this.type === "npc") this._sizeToken();
  ...
  prepareBaseData() {
    if (this.type === "player" || this.type === "npc") this._initializeData();
  }
  ...
  _computeSkillTotals() {
    for (let skillKey in this.skills) {
      ...
      skill.value =
        skill.training +
        this.attributes[game.aos.config.skillAttributes[skillKey]].value +
        skill.bonus;
    }
  }
  ...
  _computeAethericDevice(item) {
    ...
    if (item.armour > 0) this.combat.armour.value += item.armour;
  }

  _computeWeapon(item) {
    if (item.traitList.defensive) this.combat.defence.total += 2;
  }
  ...
  _applySpeedModifier(value, modifier = 0) {
    ...
    if (valueNum != 0) valueNum = Math.clamped(valueNum + modifier, 1, 3);
    else return value;
  ...
  _computeSecondary() {
    ...
    this.combat.melee.total +=
      this.attributes.body.value +
      this.skills.weaponSkill.training +
      this.combat.melee.bonus * 2;
    this.combat.accuracy.total +=
      this.attributes.mind.value +
      this.skills.ballisticSkill.training +
      this.combat.accuracy.bonus * 2;
    this.combat.defence.total +=
      this.attributes.body.value +
      this.skills.reflexes.training +
      this.combat.defence.bonus * 2;
    this.combat.armour.value += this.combat.armour.bonus;
    this.combat.initiative.total +=
      this.attributes.mind.value +
      this.skills.awareness.training +
      this.skills.reflexes.training +
      this.combat.initiative.bonus;
    this.combat.naturalAwareness.total +=
      Math.ceil(
        (this.attributes.mind.value + this.skills.awareness.training) / 2
      ) + this.combat.naturalAwareness.bonus;
    this.power.isUndercharge = this.power.consumed > this.power.capacity;

    if (this.autoCalc.toughness) {
      this.combat.health.toughness.max +=
        this.attributes.body.value +
        this.attributes.mind.value +
        this.attributes.soul.value +
        this.combat.health.toughness.bonus;
    } else if (!this.isSwarm) {
      this.combat.health.toughness.max = 1;
    }

    if (this.autoCalc.wounds) {
      this.combat.health.wounds.max +=
        Math.ceil(
          (this.attributes.body.value +
            this.attributes.mind.value +
            this.attributes.soul.value) /
            2
        ) + this.combat.health.wounds.bonus;
      this.combat.health.wounds.deadly =
        this.combat.health.wounds.value >= this.combat.health.wounds.max;
    }

    if (this.autoCalc.mettle) {
      this.combat.mettle.max +=
        Math.ceil(this.attributes.soul.value / 2) + this.combat.mettle.bonus;
    }
  }
  ...
  computeSpentExperience() {
    ...
    for (let attribute of Object.values(this.attributes)) {
      let index =
        attribute.value >= 1 && attribute.value <= 8 ? attribute.value - 1 : 0;
  ...
  async applyArchetype(archetype, apply) {
    if (this.type == "player" && apply) {
      new game.aos.apps.CharacterCreation({ actor: this, archetype }).render(
        true
      );
    } else if (this.type == "player") {
      this.update({
        "system.bio.archetype": archetype.name,
        "system.bio.species": archetype.species,
      });
      this.createEmbeddedDocuments("Item", [archetype.toObject()]);
    } else if (this.type == "npc" && apply) {
      ui.notifications.notify(
        `${game.i18n.localize("CHARGEN.APPLYING")} ${
          archetype.name
        } ${game.i18n.localize("BIO.ARCHETYPE")}`
      );
      ...
      actorData.system.combat.health.toughness.value =
        archetype.attributes.body +
        archetype.attributes.mind +
        archetype.attributes.soul;
      actorData.system.combat.mettle.value = Math.ceil(
        archetype.attributes.soul / 2
      );

      actorData.name = archetype.name;
      actorData.img = archetype.img;
      actorData.prototypeToken.texture.src = archetype.img
        .replace("images", "tokens")
        .replace("actors", "tokens");
  ...
  async setupAttributeTest(attribute, options = {}) {
    ...
    dialogData.title = `${game.i18n.localize(
      game.aos.config.attributes[attribute]
    )} ${game.i18n.localize("SKILL.TEST")}`;
  ...
  async setupSkillTest(skill, attribute, options = {}) {
    let dialogData = RollDialog._dialogData(
      this,
      attribute || game.aos.config.skillAttributes[skill],
      skill,
      options
    );
    dialogData.title = `${game.i18n.localize(
      game.aos.config.skills[skill]
    )} ${game.i18n.localize("SKILL.TEST")}`;
    let testData = await RollDialog.create(dialogData);
    testData.targets = dialogData.targets;
    testData.speaker = this.speakerData();
  ...
  async setupCombatTest(weapon, options) {
    if (typeof weapon == "string") weapon = this.items.get(weapon);
  ...
  async setupSpellTest(power) {
    if (typeof power == "string") power = this.items.get(power);
  ...
  async setupMiracleTest(power) {
    ...
    if (power.cost > this.combat.mettle.value)
      return ui.notifications.error(
        game.i18n.localize("ERROR.NotEnoughMettle")
      );

    let dialogData = RollDialog._dialogData(this, "soul", "devotion");
    dialogData.title = `${power.name} ${game.i18n.localize("POWER.TEST")}`;
    dialogData.difficulty =
      game.aos.utility.DNToObject(power.test.dn).difficulty ||
      dialogData.difficulty;
    dialogData.complexity =
      game.aos.utility.DNToObject(power.test.dn).complexity ||
      dialogData.complexity;
    let testData = await RollDialog.create(dialogData);
  ...
  async applyDamage(
    ...
    if (damage > 0 && restraining) await this.addCondition("restrained");
    ...
    let note = game.i18n.format("NOTIFICATION.APPLY_DAMAGE", {
      damage: damage,
      name: this.prototypeToken.name,
    });
    ...
    if (remaining < 0 && this.combat.health.wounds.max > 0) {
      if (ineffective) remaining = -1; // ineffective can only cause minor wounds
  ...
  async applyHealing(healing) {
    ...
    if (healing.toughness)
      updates["system.combat.health.toughness.value"] = Math.min(
        this.combat.health.toughness.value + healing.toughness,
        this.combat.health.toughness.max
      );
    else return;
    ...
    let note = game.i18n.format("NOTIFICATION.APPLY_HEALING", {
      toughness: healing.toughness,
      name: this.prototypeToken.name,
    });
    ui.notifications.notify(note);
  ...
  async computeNewWound(remaining) {
    ...
    if (
      this.combat.health.wounds.value + damage >
      this.combat.health.wounds.max
    ) {
  ...
  addWound(type = "", damage = 0) {
    if (isNaN(damage) && type) damage = game.aos.config.woundDamage[type] || 0;
  ...
  async addCondition(effect, options = {}) {
    if (typeof effect === "string")
      effect = CONFIG.statusEffects
        .concat(Object.values(game.aos.config.systemEffects))
        .find((e) => e.id == effect);
  ...
  async applyRend(damage, { magicWeapon = false } = {}) {
    let armours = this.items.filter(
      (i) =>
        ...
        (!i.traitList.sigmarite || (i.traitList.sigmarite && magicWeapon))
    ); // Only sigmarite - unless magic weapon

    if (armours.length === 0)
      return ui.notifications.notify(
        game.i18n.localize("NOTIFICATION.REND_FAIL")
      );
    ...
    let note = game.i18n.format("NOTIFICATION.APPLY_REND", {
      damage: damage,
      name: this.prototypeToken.name,
    });
    ui.notifications.notify(note);
  ...
  allDialogChanges({ targets = [] } = {}) {
    ...
    let changes = effects
      .filter((e) => !e.disabled)
      .reduce(
        (prev, current) => mergeObject(prev, current.getDialogChanges()),
        {}
      );

    if (targets.length) {
      let target = targets[0];
      let targetChanges = target.effects
        .filter((e) => !e.disabled)
        .reduce(
          (prev, current) =>
            mergeObject(prev, current.getDialogChanges({ target: true })),
          {}
        );
  ...
  async onEnterDrawing(drawing) {
    ...
    if (cover) await this.addCondition(cover, options);
    if (hazard) await this.addCondition(hazard, options);
    if (obscured) await this.addCondition(obscured, options);
    if (difficult) await this.addCondition("difficult", options);
  }
  ...
  async onLeaveDrawing(drawing) {
    ...
    if (cover) await this.removeCondition(cover);
    if (hazard) await this.removeCondition(hazard);
    if (obscured) await this.removeCondition(obscured);
    if (difficult) await this.removeCondition("difficult");
  }
    ...
    for (let t of tokens) {
      ...
      t.hud.createScrollingText(
          ...
          : change.signedString(),
        {
          ...
          fill: options.mettle
            ? "0x6666FF"
            : change < 0 || options.wound
            ? "0xFF0000"
            : "0x00FF00", // I regret nothing
          stroke: 0x000000,
          strokeThickness: 4,
          jitter: 0.25,
        }
  ...
  get autoCalc() {
    return {
      ...
      tokenSize:
        !this.isSwarm && this.getFlag("polaris-soulbound", "autoCalcTokenSize"),
    };
  }
  ...
  get Speed() {
    ...
    if (speed.flight != "none")
      display.push(
        `${game.i18n.localize("HEADER.FLY_SPEED")} (${
          game.aos.config.speed[speed.flight]
        })`
      );

    if (speed.swim != "none")
      display.push(
        `${game.i18n.localize("HEADER.SWIM_SPEED")} (${
          game.aos.config.speed[speed.swim]
        })`
      );
  ...
  get size() {
    if (this.type == "npc") return this.bio.size;
    else return 2;
  }

  // @@@@@ BOOLEAN GETTERS @@@@@
  get isSwarm() {
    return this.bio.type === 0;
  }
  ...
  get soulfire() {
    return this.system.soulfire;
  }
  get doom() {
    return this.system.doom;
  }
  get power() {
    return this.system.power;
  }
  get members() {
    return this.system.members || [];
  }
