...
export default class CharacterCreation extends FormApplication {
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      ...
      template:
        "systems/polaris-soulbound/template/apps/character-creation.hbs",
      closeOnSubmit: false,
      width: 1400,
      height: 800,
      resizable: true,
  ...
  initializeCharacter() {
    this.character = new AgeOfSigmarActor({
      type: "player",
    ...
    if (this.archetype.skills.core) {
      this.character.updateSource({
        [`system.skills.${this.archetype.skills.core}.training`]: 1,
      });
      this.character.updateSource({
        [`system.skills.${this.archetype.skills.core}.focus`]: 1,
      });
    }
    this.character.updateSource({
      "system.bio.archetype": this.archetype.name,
    });
  ...
  async getData() {
    ...
    data.coreTalents = await Promise.all(
      this.archetype.talents.core.map(
        async (t) =>
          new AgeOfSigmarItem(
            mergeObject(
              (await game.aos.utility.findItem(t.id, "talent")).toObject(),
              t.diff,
              { overwrite: true }
            )
          )
    ...
    data.chooseTalents = await Promise.all(
      this.archetype.talents.list.map(
        async (t) =>
          new AgeOfSigmarItem(
            mergeObject(
              (await game.aos.utility.findItem(t.id, "talent")).toObject(),
  ...
  async _updateObject(ev, formData) {
    ...
    this.character.updateSource({
      [`system.attributes.body.value`]: this.archetype.attributes.body,
    });
    this.character.updateSource({
      [`system.attributes.mind.value`]: this.archetype.attributes.mind,
    });
    this.character.updateSource({
      [`system.attributes.soul.value`]: this.archetype.attributes.soul,
    });
    this.character.updateSource({
      "system.bio.species": this.archetype.species,
    });
    ...
    let talents = this.archetype.talents.core.map((t) =>
      game.aos.utility.findItem(t.id, "talent")
    );
    ...
    await this.actor.update(
      mergeObject(this.character.toObject(), { overwrite: true })
    );
  ...
  validateForm() {
    return new Promise((resolve) => {
      ...
      // TALENTS
      let talentCount = parseInt(
        this.element.find(".talent-count input")[0].value
      );
      ...
      this.element.find(".equipment-item.generic").each((i, e) => {
        if (!this.isDisabled(e)) {
          ...
          if (equipment.filters.length) unresolvedGenerics = true;
        }
      });
    ...
    return equipment
      .filter((e) => {
        let element = this.element.find(
          `.equipment-item[data-id='${e.groupId}']`
        );
      ...
      .map(async (e) => {
        ...
        if (e.type == "generic") {
          ...
          if (document)
            item = new AgeOfSigmarItem(
              mergeObject(document.toObject(), e.diff, { overwrite: true })
            );
          else {
  ...
  constructEquipmentHTML() {
    ...
    let groupToHTML = (group, { selector = false } = {}) => {
      ...
      if (["and", "or"].includes(group.type)) {
        ...
        html += `<div class='equipment-group ${
          group.type == "or" ? "choice" : ""
        } ${group.groupId == "root" ? "root" : ""}' data-id="${
          group.groupId
        }">`;
        html += group.items
          .map((g) => {
        ...
        group.type == "item" ||
        (group.type == "generic" && group.filters.length == 0)
      ) {
    ...
    html += groupToHTML(
      ArchetypeGroups.groupIndexToObjects(
        this.archetype.groups,
        this.archetype
      ),
      html
    );
  ...
  activateListeners(html) {
    ...
    html.find(".skill-button button").click((ev) => {
      ...
      if (direction == "inc") {
        if (this.character.skills[skill][type] >= 3) return;

        this.character.updateSource({
          [`system.skills.${skill}.${type}`]:
            this.character.skills[skill][type] + 1,
        });
      } else {
        if (this.character.skills[skill][type] <= 0) return;
        if (
          this.archetype.skills.core == skill &&
          this.character.skills[skill][type] <= 1
        )
          return;
      ...
      ev.target.parentElement.querySelector(".skill-value").textContent =
        this.character.skills[skill][type];
    ...
    html.find(".equipment-selector").click((ev) => {
      if (this.isDisabled(ev.currentTarget)) return;
    ...
    html.find(".equipment-item").click(async (ev) => {
      ...
      if (equipment.type == "generic" && equipment.filters.length) {
        ...
        new AgeOfSigmarItem(
          (await game.aos.utility.findItem(equipment.id)).toObject()
  ...
  updateExperience() {
    ...
    spentInput.value =
      this.character.experience.spent - (this.archetype.skills.core ? 2 : 0); // -2 to account for the core skill already advanced
    if (parseInt(spentInput.value) > availableXP)
      spentInput.classList.add("error");
    else spentInput.classList.remove("error");
  }
  ...
  toggleSelector(element) {
    if (this.isOn(element)) return this.setSelector(element, "off");
    else if (!this.isOn(element)) return this.setSelector(element, "on");
  }
