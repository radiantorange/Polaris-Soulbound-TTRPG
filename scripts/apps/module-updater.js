export default class ModuleUpdater extends Dialog {
  constructor(module, html) {
    super({
      title: `${game.i18n.localize("UpdaterTitle1")} ${
        module.title
      } ${game.i18n.localize("UpdaterTitle2")}`,
      content: html,
      module,
      buttons: {
        update: {
          ...
          callback: (html) => {
            if (!game.settings.get(module.id, "initialized"))
              return ui.notifications.notify(
                game.i18n.localize("UPDATER.Error")
              );
  ...
  static async create(module) {
    let html = await renderTemplate(
      "systems/polaris-soulbound/template/apps/module-updater.hbs",
      module
    );
  ...
  getUpdateSettings(html) {
    ...
    updateSettings.excludeNameChange = html
      .find('[name="excludeNameChange"]')
      .is(":checked");
  ...
  async updateDocuments(documents, settings) {
    ...
    for (let document of documents) {
      ...
      if (game[document.collectionName].has(document.id)) {
        ...
        if (
          !settings.excludeNameChange ||
          (settings.excludeNameChange && document.name == existingDoc.name)
        ) {
  ...
  async getDocuments() {
    ...
    let packs = module.data.flags.initializationPacks.map((i) =>
      game.packs.get(i)
    );
    let documents = {
      actors: [],
