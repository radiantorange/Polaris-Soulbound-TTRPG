export default class BugReportFormSoulbound extends Application {
  constructor(app) {
    ...
    this.endpoint =
      "https://aa5qja71ih.execute-api.us-east-2.amazonaws.com/Prod/soulbound";
    this.github =
      "https://api.github.com/repos/moo-man/AoS-Soulbound-FoundryVTT/";
  ...
  async _render(...args) {
    ...
    this.element
      .find(".module-check")
      .replaceWith(this.formatVersionWarnings());
  }
    ...
    fetch(this.endpoint, {
      ...
      .then((res) => {
        if (res.status == 201) {
          ...
          res.json().then((json) => {
            console.log(
              `Thank you for your submission. If you wish to monitor or follow up with additional details like screenshots, you can find your issue here: ${json.html_url}`
            );
          });
        } else {
  ...
  async checkVersions() {
    ...
    for (let key of this.domainKeys) {
      if (key == game.system.id) {
        ...
        let release = await fetch(this.github + "releases/latest")
          .then((r) => r.json())
          .catch((e) => console.error(e));
        latest[key] = !isNewerVersion(release.tag_name, game.system.version);
      } else if (game.modules.get(key)) {
        let manifest = await fetch(
          `https://foundry-c7-manifests.s3.us-east-2.amazonaws.com/${key}/module.json`
        )
          .then((r) => r.json())
          .catch((e) => console.error(e));
        latest[key] = !isNewerVersion(
          manifest.version,
          game.modules.get(key).version
        );
      }
    }
    return latest;
  ...
  matchIssues(text) {
    ...
    this.issues.forEach((issue, issueIndex) => {
      let issueWords = (issue.title + " " + issue.body)
        .toLowerCase()
        .trim()
        .split(" ");
      words.forEach((word) => {
  ...
  showMatchingIssues(element, issues) {
    if (!issues || issues?.length <= 0) element[0].style.display = "none";
    else {
      ...
      list.append(
        issues.map(
          (i) => `<div class="issue"><a href="${i.html_url}">${i.title}</div>`
        )
      );
    }
  }
  ...
  async loadIssues() {
    let issues = await fetch(this.github + "issues")
      .then((r) => r.json())
  ...
  activateListeners(html) {
    ...
    html.find(".issuer").keyup((ev) => {
      publicityWarning.style.display = ev.currentTarget.value.includes("@")
        ? "block"
        : "none";
    });
      ...
      if (ev.currentTarget.value == "bug") {
        if (
          game.modules.contents
            .filter((i) => i.active)
            .map((i) => i.id)
            .filter((i) => !this.domainKeys.includes(i)).length > 0
        )
    ...
    html.find(".bug-submit").click((ev) => {
      ...
      if (!data.domain || !data.title || !data.description)
        return ui.notifications.error(
          game.i18n.localize("BugReport.ErrorForm")
        );
      if (!data.issuer)
        return ui.notifications.error(
          game.i18n.localize("BugReport.ErrorName1")
        );
      ...
      data.labels = [
        this.domainKeysToLabel[this.domainKeys[Number(data.domain)]],
      ];
      ...
      let officialModules = Array.from(game.modules).filter((m) =>
        this.domainKeys.includes(m.id)
      );
      ...
      data.description += `<br/>Active Modules: ${game.modules.contents
        .filter((i) => i.active)
        .map((i) => i.id)
        .filter((i) => !this.domainKeys.includes(i))
        .join(", ")}`;
