...
export default class ArchetypeGeneric extends FormApplication {
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      ...
      template: "systems/polaris-soulbound/template/apps/archetype-generic.hbs",
      height: "auto",
      width: 285,
  ...
  getData() {
    ...
    if (Number.isNumeric(this.object.index)) {
      generic.property =
        this.object.item.equipment[this.object.index]?.property;
  ...
  async _updateObject(event, formData) {
    ...
    let filters = Array.from($(event.target).find(".test")).map((t) => {
      return { test: t.value };
    });

    // Add Property to each element in the filter array
    Array.from($(event.target).find(".property")).map(
      (p, i) => (filters[i].property = p.value)
    );

    // Add value to each element in the filter array
    Array.from($(event.target).find(".value")).map(
      (p, i) => (filters[i].value = p.value)
    );
    ...
    if (Number.isNumeric(this.object.index))
      ...
      groups = this.object.item.addToGroup({
        type: "item",
        index: equipment.length - 1 || 0,
      });
    }

    this.object.item.update({
