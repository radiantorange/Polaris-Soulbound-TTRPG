export default class FilterResults extends FormApplication {
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      ...
      template: "systems/polaris-soulbound/template/apps/filter-results.hbs",
      width: 300,
      height: 800,
      resizable: true,
    });
  ...
  async getAllItems() {
    ...
    let packs = await Promise.all(
      game.packs
        ...
        .map((i) => i.getDocuments())
    );

    packs.forEach((p) => {
      // Remove duplicates (match by ID, don't show compendium item if world item already listed)
      items = items.concat(
        p.filter((i) => !items.find((existing) => existing.id == i.id))
      );
    });
  ...
  applyFilters(items, filters) {
    filters.forEach((f) => {
      items = items.filter((i) => {
        ...
        if ([propValue, test, testValue].includes(undefined)) return false;
        return (0, eval)(
          `"${propValue}" ${this.comparisons[test]} "${testValue}"`
        );
      });
    });
    return items;
