export default class ItemTraits extends FormApplication {
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      id: "item-traits",
      template: "systems/polaris-soulbound/template/apps/item-traits.hbs",
      height: "auto",
      width: 285,
  ...
  _updateObject(event, formData) {
    ...
    for (let key in formData) {
      if (key == "custom-traits")
        ...
        if (formData[`${key}-value`]) traitObj.value = formData[`${key}-value`];
  ...
  constructCustomString(traits) {
    ...
    if (typeof traits == "string") return customString;
