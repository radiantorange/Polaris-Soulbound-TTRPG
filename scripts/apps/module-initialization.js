export default class ModuleInitializer extends Dialog {
        ...
        initialize: {
          ...
          callback: async () => {
            ...
            ui.notifications.notify(
              game.modules.get(module).title + ": Initialization Complete"
            );
          },
        },
        update: {
          ...
          callback: async () => {
            let updater = await game.aos.apps.ModuleUpdater.create(
              game.modules.get(module),
              this
            );
            updater.render(true);
          },
  ...
  async createOrUpdateDocuments(documents, collection) {
    ...
    if (existingDocuments.length) {
      console.log("Pre Existing Documents: ", null, {
        args: existingDocuments,
      });
      existingDocuments = await new Promise((resolve) =>
        new ModuleDocumentResolver(existingDocuments, { resolve }).render(true)
      );
      console.log("Post Existing Documents: ", null, {
        args: existingDocuments,
      });
    }
  ...
  async deleteModuleContent(id) {
    ...
    if (proceed) {
      ...
      let moduleActors = game.actors.filter(
        (doc) => doc.flags[id] && !doc.hasPlayerOwner
      );
...
class ModuleDocumentResolver extends FormApplication {
  static get defaultOptions() {
    ...
    options.width = 400;
    options.template =
      "systems/polaris-soulbound/template/apps/document-resolver.hbs";
