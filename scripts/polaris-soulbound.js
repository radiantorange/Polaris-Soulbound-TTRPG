...
Hooks.once("init", () => {
  ...
  Actors.registerSheet("polaris-soulbound", PlayerSheet, {
    types: ["player"],
    makeDefault: true,
  });
  Actors.registerSheet("polaris-soulbound", NpcSheet, {
    types: ["npc"],
    makeDefault: true,
  });
  Actors.registerSheet("polaris-soulbound", PartySheet, {
    types: ["party"],
    makeDefault: true,
  });
  Items.unregisterSheet("core", ItemSheet);
  Items.registerSheet("polaris-soulbound", AgeOfSigmarItemSheet, {
    makeDefault: true,
  });
  DocumentSheetConfig.registerSheet(
    ActiveEffect,
    "polaris-soulbound",
    AgeOfSigmarEffectSheet,
    { makeDefault: true, label: "Soulbound Active Effect Config" }
  );
  DocumentSheetConfig.registerSheet(
    JournalEntryPage,
    "polaris-soulbound",
    Level4TextPageSheet,
    { types: ["text"], makeDefault: true, label: "Soulbound Journal Sheet" }
  );
