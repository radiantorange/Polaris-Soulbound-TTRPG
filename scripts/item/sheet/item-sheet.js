...
export class AgeOfSigmarItemSheet extends ItemSheet {
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      ...
      dragDrop: [
        ...
        { dragSelector: ".journal-list .journalentry", dropSelector: null },
      ],
  ...
  async _onDrop(ev) {
    ...
    if (!dropDocument) return;

    if (this.item.type === "archetype") {
      if (
        dropDocument.documentName == "JournalEntry" ||
        dropDocument.documentName == "JournalEntryPage"
      ) {
      ...
      if (dropDocument.type == "talent") {
        ...
        ["weapon", "armour", "equipment", "aethericDevice", "rune"].includes(
          dropDocument.type
        )
      ) {
        ...
        let groups = this.item.addToGroup({
          type: "item",
  ...
  async getData() {
    ...
    if (this.item.archetype) {
      let list = duplicate(
        getProperty(this.item.archetype.data, this.item.archetypeItemPath)
      );
    ...
    if (this.item.type == "archetype") {
      ...
      let element = $(
        ArchetypeGroups.constructHTML(data.item, {
          parentheses: true,
          commas: true,
          draggable: false,
        })
  ...
  async _handleEnrichment() {
    let enrichment = {};
    enrichment["system.description"] = await TextEditor.enrichHTML(
  ...
  activateListeners(html) {
    ...
    html.find(".effect-create").click(async (ev) => {
      if (this.item.isOwned)
        ui.notifications.error(
          "Effects can only be added to world items or actors directly"
        );
    ...
    html.find(".condition-toggle").click((ev) => {
      ...
      if (this.item.hasCondition(key)) this.item.removeCondition(key);
      else this.item.addCondition(key);
    });
    ...
    html.find(".entry-delete").click((ev) => {
      let index = parseInt(
        $(ev.currentTarget).parents(".entry-element").attr("data-index")
      );
      let path = $(ev.currentTarget)
        .parents(".entry-element")
        .attr("data-path");
      ...
      if (obj) {
        if (ev.button == 0) {
          if (obj.type == "generic")
            ...
            if (!item) item = await game.aos.utility.findItem(obj.id);

            if (!item) throw Error("Could not find Item reference");

            new AgeOfSigmarItem(item.toObject(), {
              archetype: { item: this.item, index, path },
            }).sheet.render(true, { editable: this.isEditable });
          }
        } else {
          new Dialog({
            title: "Delete Item?",
            content:
              "Do you want to remove this item from the Archetype? This will reset the groupings.",
            buttons: {
              yes: {
                label: "Yes",
                callback: async () => {
