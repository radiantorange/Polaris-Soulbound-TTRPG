...
export class AgeOfSigmarItem extends Item {
  ...
  async update(data = {}, context = {}) {
    ...
    if (this.archetype) {
      ...
      let list = duplicate(
        getProperty(this.archetype.data, this.archetypeItemPath)
      );
      let item = list[this.archetypeItemIndex];
      mergeObject(
        // Merge current diff with new diff
        item.diff,
      ...
      if (item.diff.name) item.name = item.diff.name;
  ...
  prepareOwnedData() {
    let functionName = `prepareOwned${
      this.type[0].toUpperCase() + this.type.slice(1)
    }`;

    if (this[functionName]) this[functionName]();

    if (this.isAttack) this.prepareAttack();
  }
  ...
  resetGroups() {
    this.update({
      "system.groups": {
        type: "and",
        groupId: "root",
        items: Array.fromRange(this.equipment.length).map((i) => {
          return { type: "item", index: i, groupId: randomID() };
        }),
      },
  ...
  async sendToChat() {
    ...
    const html = await renderTemplate(
      "systems/polaris-soulbound/template/chat/item.hbs",
      { item, data: item.system }
    );
    const chatData = {
      user: game.user.id,
  ...
  get traitList() {
    let traits = {};
    if (!this.traits || !Array.isArray(this.traits)) return [];
  ...
  get Test() {
    let test = this.test;
    return `DN ${test.dn} ${game.aos.config.attributes[test.attribute]} (${
      game.aos.config.skills[test.skill]
    })`;
  }
  ...
  get Duration() {
    if (this.type == "spell")
      return `${this.duration.value} ${
        game.aos.config.durations[this.duration.unit]
      }${this.duration.value > 0 ? "s" : ""}`;
    else if (this.type == "miracle") return this.duration;
  }
  ...
  get Type() {
    if (this.type == "armour") return game.aos.config.armourType[this.subtype];
  }
  ...
  get Category() {
    ...
    if (this.type == "armour") return game.aos.config.armourType[this.category];
  ...
  async GetArchetypeItems() {
    ...
    items = items.concat(
      talents.map(async (t) => {
        let item = (
          await game.aos.utility.findItem(t.id, "talent")
        )?.toObject();
        if (item) mergeObject(item, t.diff, { overwrite: true });
        return item;
      })
    );
    ...
    // Get all archetype talents, merge with diff
    items = items.concat(
      this.equipment.map(async (i) => {
        let item = (
          await game.aos.utility.findItem(i.id, "equipment")
        )?.toObject();
        if (item) mergeObject(item, i.diff, { overwrite: true });
        return item;
      })
    );
  ...
  get equippable() {
    return hasProperty(this, "system.equipped");
  }
  // @@@@@@ TYPE GETTERS @@@@@@
  /************** ITEMS *********************/
  get isTalent() {
  ...
  get isMiracle() {
    return this.type === "miracle";
  }
  get isPower() {
    return this.isSpell || this.isMiracle;
  }
  /************** PARTY ITEMS *********************/
  get isShortGoal() {
    return this.type === "goal" && this.subtype === "short";
  }
  get isLongGoal() {
    return this.type === "goal" && this.subtype === "long";
  }
  get isAlly() {
  ...
  get isRumour() {
    return this.type === "rumour";
  }
  get isFear() {
    return this.type === "fear";
  }
  get isThreat() {
  ...
  get isAethericDevice() {
    return this.type === "aethericDevice";
  }
  get isAttack() {
    return this.isWeapon || (this.isAethericDevice && this.damage);
  }
  get isRune() {
  ...
  get hasTest() {
    if (this.type == "miracle" && (!this.test.attribute || !this.test.skill))
      ...
      this.type != "miracle" &&
      (!this.test || !this.test.dn || !this.test.dn.includes(":"))
    )
      return false;
    if (!game.aos.config.attributes[this.test.attribute]) return false;
    return true;
  }
  ...
  get requirements() {
    return this.system.requirements;
  }
  get crafting() {
    return this.system.crafting;
  }
  get damage() {
  ...
  get overcasts() {
    return this.system.overcasts;
  }
  get lore() {
    return this.system.lore;
  }
  get requirement() {
    return this.system.requirement;
  }
  get category() {
  ...
  get journal() {
    return this.system.journal;
  }
  get free() {
    return this.system.free;
  }
