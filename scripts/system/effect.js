...
export default class AgeOfSigmarEffect extends ActiveEffect {
  ...
  apply(actor, change) {
    if (change.value.includes("@")) {
      ...
      else actor.derivedEffects.push(change);
    } else {
  ...
  fillDerivedData(actor, change) {
    ...
    let matches = Array.from(
      change.value.matchAll(/@UUID\[Actor\.(.+?)\]\.system\.(.+)/gm)
    );
    ...
    if (matches[0]) {
      ...
      if (!actor) return console.error(`ERROR.ReferencedActorNotFound`);
      change.value = "@" + path;
    }

    let data = (0, eval)(
      Roll.replaceFormulaData(change.value, actor.getRollData())
    );
  ...
  get item() {
    if (this.parent && this.parent.documentName == "Item") return this.parent;
    else if (this.origin && this.parent.documentName == "Actor") {
      let origin = this.origin.split(".");
      if (this.parent && origin[1] == this.parent?.id) {
        // If origin ID is same as parent ID
        if (origin[3]) {
  ...
  static async populateEffectData(effectData, test, item) {
    ...
    for (let change of effectData.changes) {
      ...
      if (Number.isNumeric(value)) change.value = parseInt(value);
      else if (!change.value.includes("@UUID")) change.value = 0;
    }
  ...
  get sourceName() {
    if (!this.origin) return super.sourceName;
  ...
  get isCondition() {
    return CONFIG.statusEffects
      .map((i) => i.id)
      .includes(Array.from(this.statuses)[0]);
  }
