...
export default class CombatTest extends Test {
  ...
  computeDualResult(type) {
    ...
    if (type == "primary")
      result.dice = duplicate(
        this.result.dice.filter((i) => i.index < testData.pool)
      );
    else
      result.dice = duplicate(
        this.result.dice.filter(
          (i) => i.index >= this.testData.dualWieldingData.primary.pool
        )
      );
    ...
    result.degree = result.success
      ? result.successes - testData.dn.complexity
      : testData.dn.complexity - result.successes;
    result.triggers = result.dice.filter((die) => die.value === 6).length;
    result.dn = testData.dn;

    if (result.success)
      result.damage = this._computeDamageWithWeapon(
        result,
        type == "primary" ? this.weapon : this.secondaryWeapon
      );
  ...
  get template() {
    if (this.isDualWielding)
      return "systems/polaris-soulbound/template/chat/weapon/weapon-dual-roll.hbs";
    else
      return "systems/polaris-soulbound/template/chat/weapon/weapon-roll.hbs";
  }
  ...
  _computeDamageWithWeapon(result, weapon) {
    ...
    if (weapon.traitList.cleave) {
      ...
      effect.text = game.i18n.format("TRAIT.CLEAVE_EFFECT", {
    ...
    if (weapon.traitList.rend) {
      effect = this._createTraitEffect();
      (effect.isRend = true),
        (effect.text = game.i18n.format("TRAIT.REND_EFFECT", {
          triggers: result.triggers,
        }));
      damage.traitEffects.push(effect);
    }
  ...
  get secondaryWeapon() {
    return this.actor.items.get(
      this.testData.dualWieldingData?.secondary?.itemId
    );
  }
