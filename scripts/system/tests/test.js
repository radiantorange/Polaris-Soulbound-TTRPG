export default class Test {
  ...
  async rollTest() {
    this.roll = this.testData.roll
      ? Roll.fromData(this.testData.roll)
      : new Roll(`${this.numberOfDice}d6cs>=${this.testData.dn.difficulty}`);
  ...
  computeResult() {
    ...
    result.degree = result.success
      ? result.successes - this.testData.dn.complexity
      : this.testData.dn.complexity - result.successes;
  ...
  _computeRoll() {
    ...
    result.focus =
      (this.testData.doubleFocus ? result.focus * 2 : result.focus) +
      this.testData.bonusFocus;
    ...
    if (this.context.rerolled) {
      ...
      sorted = sorted.map((die) => {
        ...
        else return sorted.find((i) => i.index == index);
      });
    }
    ...
    for (let i = 0; i < sorted.length; i++) {
      let die = {
        ...
        highlight: false,
        success: false,
        rerolled: this.context.rerolled
          ? this.testData.shouldReroll[sorted[i].index]
          : false,
      };
      if (this.testData.allocation[die.index] > 0) {
        ...
        if (die.value > 6) die.value = 6;
        die.highlight = true;
      }
      if (die.value >= dn.difficulty) die.success = true;

      if (die.value == 6) die.trigger = true;
      result.dice.push(die);
    }

    result.triggers = result.dice.filter((die) => die.value === 6).length;
    result.successes = result.dice.reduce(
      (prev, current) => (prev += current.success),
      0
    );
    ...
    result.dice.forEach((die) => {
      ...
      if (die.success) die.img = `${path}/dice-${die.value}-chat.webp`;
      if (die.highlight) die.img = `${path}/dice-${die.value}-highlight.webp`;
    });
  ...
  allocateFocus(allocation) {
    if (
      allocation.reduce((prev, current) => prev + current, 0) >
      this.result.focus
    )
  ...
  async reroll(shouldReroll) {
    ...
    if (game.dice3d) {
      ...
      dsnRerollData.terms[0].results = dsnRerollData.terms[0].results.filter(
        (die) => this.testData.shouldReroll[die.index]
      );
  ...
  async sendToChat({ newMessage = false } = {}) {
    ...
    chatData.speaker.alias = this.actor.token
      ? this.actor.token.name
  ...
  static _getSortedDiceFromRoll(roll) {
    return roll.terms
      ...
      .map((die) => {
        return { result: die.result, index: die.index };
      })
  ...
  get targets() {
    return this.context.targetSpeakers.map((speaker) =>
      game.aos.utility.getSpeaker(speaker)
    );
  }
  ...
  get numberOfDice() {
    ...
    if (this.skill)
      num +=
        (this.testData.doubleTraining
          ? this.skill.training * 2
          : this.skill.training) + this.skill.bonus;
    return num;
  }
  ...
  _itemTest(item) {
    let DN = item.test.dn;
    if (DN.includes("S")) DN = DN.replace("S", 1 + this.result.degree);
