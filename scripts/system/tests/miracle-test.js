...
export default class MiracleTest extends Test {
  get template() {
    return "systems/polaris-soulbound/template/chat/miracle/miracle-roll.hbs";
  }
    ...
    if (!this.context.mettleSubtracted) {
      this.actor.update({
        "system.combat.mettle.value":
          this.actor.combat.mettle.value - this.item.cost,
      });
  ...
  _ItemTestDisplay(item) {
    return `DN 4:1 ${game.aos.config.attributes[item.test.attribute]} (${
      game.aos.config.skills[item.test.skill]
    })`;
  }
}
