...
export class RollDialog extends Dialog {
  ...
  static async create(data) {
    ...
    return new Promise(async (resolve, reject) => {
      const html = await renderTemplate(
        "systems/polaris-soulbound/template/dialog/common-roll.hbs",
        data
      );
      return new this(
        {
            ...
            roll: {
              ...
              callback: async (html) => {
                ...
                data.dn = {
                  ...
                  name: `${game.aos.config.attributes[data.attribute]} ${
                    data.skill
                      ? "(" + game.aos.config.skills[data.skill] + ")"
                      : ""
                  }`,
                };
                resolve(data);
  ...
  static _dialogData(actor, attribute, skill, options = {}) {
    return {
      ...
      changes: actor.allDialogChanges({
  ...
  static runConditional(type, data) {
    ...
    for (let id in data.changes) {
      ...
      try {
        let func = new Function("data", change.conditional[type]).bind({
          actor: data.actor,
          targets: data.targets,
          effect: change.document,
        });
        results[id] = func(data) == true; // Only accept true returns
      } catch (e) {
        console.error(
          "Something went wrong when processing conditional dialog effect: " +
            e,
          change
        );
        results[id] = false;
  ...
  applyAutomaticChanges(automatic) {
    try {
      ...
      let activatedIndex = this.data.dialogData.condensedChanges
        .map((cc, index) => {
          if (cc.id.some((id) => automaticIds.includes(id))) return index;
        })
      ...
      if (Object.values(automatic).some((i) => i)) select.focus();
    } catch (e) {
  ...
  static condenseChanges(changes) {
    ...
    for (let id in changes) {
      let existing = condensed.find(
        (i) => i.description == changes[id].conditional.description
      );
      if (existing) {
        existing.id.push(id);
        ...
        condensed.push({
          id: [id],
          description: changes[id].conditional.description,
          tooltip: [changes[id].document],
        });
      }
    }
  ...
  activateListeners(html) {
    ...
    this.inputs["double-training"] = html
      .find("#double-training")
      .change((ev) => {
  ...
  _onEffectSelect(ev) {
    ...
    for (let c of changes) {
      if (AgeOfSigmarEffect.numericTypes.includes(c.key))
        this.effectValues[c.key] =
          (this.effectValues[c.key] || 0) + parseInt(c.value);
      else if (
        c.key == "double-training" ||
        c.key == "double-focus" ||
        c.key == "triggerToDamage"
      ) {
      ...
      if (this.effectValues[input] != null) {
        if (this.inputs[input].type == "checkbox")
          ...
          if (this.bounds[input])
            this.inputs[input].value = Math.clamped(
              newVal,
              this.bounds[input][0],
              this.bounds[input][1]
            );
          else this.inputs[input].value = newVal;
        } else this.inputs[input].value = this.effectValues[input];
      } // if not part of effect values, use user entry only
      else {
        if (this.inputs[input].type == "checkbox")
          this.inputs[input].checked = this.userEntry[input];
        else this.inputs[input].value = this.userEntry[input];
      }
    }
  }
}
...
export class CombatDialog extends RollDialog {
  static async create(data) {
    ...
    return new Promise(async (resolve, reject) => {
      const html = await renderTemplate(
        "systems/polaris-soulbound/template/dialog/combat-roll.hbs",
        data
      );
      return new this(
        {
          ...
          buttons: {
            roll: {
              ...
              callback: async (html) => {
                ...
                testData.dn = this._getDn(
                  data.weapon.name,
                  testData.rating,
                  testData.targetDefence
                );
                if (testData.dualWieldingData) {
                  testData.dualWieldingData.primary.dn = this._getDn(
                    data.weapon.name,
                    testData.rating,
                    testData.dualWieldingData.primary.defence
                  );
                  testData.dualWieldingData.secondary.dn = this._getDn(
                    data.actor.items.get(
                      testData.dualWieldingData.secondary.itemId
                    ).name,
                    testData.rating,
                    testData.dualWieldingData.secondary.defence
                  );
                }
  ...
  static extractDialogData(html) {
    ...
    data.combat = {
      armour: html.find("#armour")[0].value,
      bonusDamage: parseInt(html.find("#bonusDamage")[0].value),
    };
  ...
  static _dialogData(actor, weapon) {
    ...
    data.combat = {
      ...
      skill: skill,
      swarmDice:
        actor.type === "npc" && actor.isSwarm
          ? actor.combat.health.toughness.value
          : 0,
      bonusDamage: 0,
    };

    data.showDualWielding =
      actor.items.filter((i) => i.isAttack && i.equipped).length >= 2;
    data.weapon = weapon;
    data.otherWeapons = actor.items.filter(
      (i) => i.isAttack && i.equipped && i.id != weapon.id
    );

    let targets = Array.from(game.user.targets);
    const hasTarget = targets.length; // No additinal Input when target function is used
    data.attackRating =
      weapon.category === "melee" ? data.combat.melee : data.combat.accuracy;
    ...
    if (hasTarget) {
      ...
      } // Populate secondary target with the same as the primary target
      else data.secondaryTarget = duplicate(data.primaryTarget);
    }
  ...
  activateListeners(html) {
    ...
    html
      .find("#attribute, #skill, #double-training, #bonusDice, .effect-select")
      .change((ev) => {
    ...
    html.find(".pool").change((ev) => {
      ...
      if (inputValue > poolSize) {
        ...
        return ui.notifications.error(
          game.i18n.localize("DIALOG.POOL_INPUT_ERROR")
        );
      }

      let otherInput;
      if (isPrimary) otherInput = dialog.find("[name='secondary-pool']")[0];
      // if isSecondary
      else otherInput = dialog.find("[name='primary-pool']")[0];
  ...
  toggleDualWielding(ev) {
    ...
    if (checked) {
      ...
      $(ev.currentTarget).parents(".app")[0].style.height =
        parseInt($(ev.currentTarget).parents(".app")[0].style.height) +
        110 +
        "px";
      ...
      if (this.data.targets.length == 1)
        ...
        )[0].textContent = `(${this.data.targets[0].name})`;
  ...
  numberOfDice() {
    ...
    if (skill) {
      ...
      num +=
        (doubleTraining ? skillObject.training * 2 : skillObject.training) +
        skillObject.bonus;
    }
    return num;
  }
...
export class SpellDialog extends RollDialog {
  static async create(data) {
    ...
    return new Promise(async (resolve, reject) => {
      const html = await renderTemplate(
        "systems/polaris-soulbound/template/dialog/spell-roll.hbs",
        data
      );
      return new this(
        {
  ...
  static _dialogData(actor, spell) {
    ...
    data.spell = spell;
    if (game.settings.get("polaris-soulbound", "loseTarget")) {
      game.user.updateTokenTargets([]);
    }
    return data;
  }
