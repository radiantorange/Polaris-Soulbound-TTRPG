export default class Migration {
  static async checkMigration() {
    ...
    let systemMigrationVersion = game.settings.get(
      "polaris-soulbound",
    ...
    if (
      !systemMigrationVersion ||
      foundry.utils.isNewerVersion(migrationTarget, systemMigrationVersion)
    ) {
      this.migrateWorld();
    }
  ...
  static async migrateWorld() {
    console.log(
      `Applying AOS:Soulbound System Migration for version ${game.system.version}. Please be patient and do not close your game or shut down your server.`
    );
  ...
  static migrateItem(item) {
    ...
    if (item.category == "range") updateData["system.category"] = "ranged";
  ...
  static migrateEffect(effect) {
    ...
    effectData.changes.forEach((c, i) => {
      if (c.mode == 0) {
        if (c.key.includes("target")) c.mode = 7;
        else c.mode = 6;
        setProperty(
          effectData,
          `flags.polaris-soulbound.changeCondition.${i}`,
          { description, script: "" }
        );
      }
    });
  ...
  static migrateTableData(table) {
    ...
    for (let result of table.results) {
      if (result.type == 0) {
        ...
        result.type == 2 &&
        this.v10Conversions[result.documentCollection]
      ) {
        updateData.results.push({
          _id: result.id,
          documentCollection: this.v10Conversions[result.documentCollection],
        });
      }
    }
  }
